    ...
    <footer className="mt-16 bg-brand-900 text-brand-100">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <a href={site.instagram} target="_blank" rel="noreferrer" className="mt-4 inline-block text-sm underline">Instagram</a>
        </div>
        <div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            <li>WhatsApp: <a className="underline" href={`https://wa.me/${site.whatsapp}`} target="_blank" rel="noreferrer">clique aqui</a></li>
            <li>E-mail: <a className="underline" href={`mailto:${site.email}`}>{site.email}</a></li>
      ...
      <div className="border-t border-white/10 py-5 text-center text-xs text-brand-300">
        © {new Date().getFullYear()} {site.name} · CNPJ {site.cnpj} · {site.address}
      </div>
    </footer>
  );
}
